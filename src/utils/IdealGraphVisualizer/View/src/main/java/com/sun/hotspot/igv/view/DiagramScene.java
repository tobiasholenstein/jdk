...
public class DiagramScene extends ObjectScene implements DiagramViewer, DoubleClickHandler {
    ...
    private boolean isVisibleBlockConnection(BlockConnection c) {
        Widget src = getWidget(c.getFromCluster());
        Widget dest = getWidget(c.getToCluster());
        return src.isVisible() && dest.isVisible();
    }

    private void doNewLayout(Set<Figure> visibleFigures, Set<FigureConnection> visibleConnections) {

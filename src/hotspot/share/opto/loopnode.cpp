...
void PhaseIdealLoop::loop_opts_skip_split_ifs(LoopOptsMode mode) {
...
void PhaseIdealLoop::build_and_optimize(LoopOptsMode mode) {
  ...
  bool _do_split_ifs = (mode == LoopOptsDefault);
  bool _skip_loop_opts = (mode == LoopOptsNone);
  bool _do_max_unroll = (mode == LoopOptsMaxUnroll);
  ...
  if (ReassociateInvariants && !C->major_progress()) {
    ...
    for (LoopTreeIterator iter(_ltree_root); !iter.done(); iter.next()) {
      ...
      if (SplitIfBlocks && _do_split_ifs &&
  ...
  if (!C->major_progress() && SplitIfBlocks && _do_split_ifs) {
    visited.clear();
